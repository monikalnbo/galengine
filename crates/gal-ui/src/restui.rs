...
pub fn draw(
    ...
    let msg = if conf.game.shutdown_message.is_empty() {
        "晚安。"
    } else {
        &conf.game.shutdown_message
    };
    ...
    canvas.copy(
        tex,
        None,
        Some(Rect::new((LOGICAL_W - q.width) as i32 / 2, 240, q.width, q.height)),
    )?;
    ...
    canvas.copy(
        tex,
        None,
        Some(Rect::new((LOGICAL_W - q.width) as i32 / 2, 320, q.width, q.height)),
    )?;
