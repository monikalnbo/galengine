...
pub fn catalog() -> Vec<String> {
    let dir = format!("{}/bgimage", gal_config::data_dir());
...
pub fn draw_grid(
    ...
    for i in 0..per_page() {
        ...
        let tex = fonts.render_text(
            FONT,
            if open { Color::RGB(200, 206, 220) } else { Color::RGB(110, 116, 132) },
            &label,
        )?;
        let q = tex.query();
        canvas.copy(tex, None, Some(Rect::new(r.x + 8, r.bottom() - 34, q.width, q.height)))?;
    }
    Ok(())
}
