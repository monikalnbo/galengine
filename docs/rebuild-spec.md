...
> 给重构者（AI 或人类）：本文是唯一需求来源。旧实现（本仓库 src/ 5100 行）可参考但**不要求沿用**——所有决策以本文为准。首个客户游戏《第八个夏天》（monikalnbo/eighth-summer，剧本/资产仓）。
...
- 旧仓 /root/galgame/engine/external 243M（旧引擎第三方库）待用户拍板删除
