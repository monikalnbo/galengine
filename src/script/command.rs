    ...
    pub fn parse(kind: &LineKind, ctx: &str) -> Result<Command, String> {
        ...
        match name {
            ...
            "char" => match tokens.as_slice() {
                [l, s] => Ok(Command::Char {
                    layer: l.parse::<u8>().ok().filter(|n| *n < 3).ok_or(format!("{ctx}：立绘层必须是 0-2"))?,
                    storage: s.to_string(),
                }),
                _ => err("char 用法：char 层(0-2) 素材|hide"),
            },
            "cg" => match tokens.as_slice() {
                [s, ms] if s != &"hide" => Ok(Command::Cg { storage: s.to_string(), fade_ms: num(ms)? }),
            ...
            "name" => match rest.split_once(' ') {
                Some((n, text)) => Ok(Command::Name { name: n.to_string(), text: text.trim().into() }),
                None => err("name 用法：name 名字 台词"),
            },
            "flag" => {
                let (var, op, val) = match tokens.as_slice() {
                    [v, op, val] if *op == "+" || *op == "-" => (v.to_string(), op.to_string(), val.to_string()),
            ...
            "set" => {
                let (v, after) = rest.split_once('=').ok_or(format!("{ctx}：set 用法：set 变量 = 表达式"))?;
            ...
            "if" => match tokens.as_slice() {
                [v, op, val, t] if ["==", "!=", ">=", "<=", ">", "<"].contains(op) => Ok(Command::If {
                    var: v.to_string(),
                    op: op.to_string(),
                    val: val.to_string(),
            ...
            "input" => {
                let (v, tail) = rest.split_once(' ').ok_or(format!("{ctx}：input 用法：input 变量 提示|宽度|默认值"))?;
            ...
            "reach" => match tokens.as_slice() {
                [s] if s == &"hide" => Ok(Command::Reach { storage: "hide".into(), dur_ms: 600, scale: 1.0 }),
                ...
                [s, ms, sc] => Ok(Command::Reach { storage: s.to_string(), dur_ms: num(ms)?, scale: num::<f32>(sc)? }),
            ...
            "window_fx" => match tokens.as_slice() {
                ...
                ["title", ..] => Ok(Command::WindowFxTitle(rest.trim_start_matches("title").trim().into())),
...
mod tests {
    ...
    #[test]
    fn 基本指令解析() {
        assert!(matches!(cmd("bg", "a 800").unwrap(), Command::Bg { storage, fade_ms: 800 } if storage == "a"));
        assert!(matches!(cmd("char", "0 hide").unwrap(), Command::Char { layer: 0, ref storage } if storage == "hide"));
        assert!(matches!(cmd("char", "3 x").is_err(), true));
        assert!(matches!(cmd("flag", "a +2").unwrap(), Command::Flag { var, op: '+', val: 2 } if var == "a"));
        assert!(matches!(cmd("flag", "a -1").unwrap(), Command::Flag { op: '-', val: 1, .. }));
        assert!(matches!(cmd("reach", "hide").unwrap(), Command::Reach { ref storage, .. } if storage == "hide"));
