...
pub fn draw(
    ...
    canvas.copy(
        tex,
        None,
        Some(Rect::new((LOGICAL_W as i32 - q.width as i32) / 2, TOP_Y as i32 - 56, q.width, q.height)),
    )?;
    for (i, (text, _)) in items.iter().enumerate() {
        let r = item_rect(i);
        let hot = i == selected;
        canvas.set_draw_color(if hot { Color::RGBA(64, 84, 140, 225) } else { Color::RGBA(12, 16, 32, 205) });
        canvas.fill_rect(r)?;
        canvas.set_draw_color(if hot { Color::RGBA(150, 200, 255, 220) } else { Color::RGBA(120, 130, 160, 110) });
        canvas.draw_rect(r)?;
