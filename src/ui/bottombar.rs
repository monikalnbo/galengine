...
pub fn draw(canvas: &mut Canvas<Window>, fonts: &mut FontBook, auto_on: bool, ff_on: bool) -> Result<(), String> {
    canvas.set_blend_mode(BlendMode::Blend);
    for (i, label) in LABELS.iter().enumerate() {
        let r = item_rect(i);
        let on = (i == 0 && auto_on) || (i == 1 && ff_on);
        canvas.set_draw_color(if on { Color::RGBA(52, 74, 128, 235) } else { Color::RGBA(28, 34, 58, 215) });
        canvas.fill_rect(r)?;
        canvas.set_draw_color(if on { Color::RGBA(160, 200, 255, 220) } else { Color::RGBA(120, 138, 185, 150) });
        canvas.draw_rect(r)?;
        let tex = fonts.render_text(19, if on { Color::RGB(255, 255, 255) } else { Color::RGB(205, 212, 230) }, label)?;
        let q = tex.query();
        canvas.copy(
            tex,
            None,
