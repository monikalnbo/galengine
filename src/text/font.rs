...
impl<'a> FontBook<'a> {
    ...
    /// 渲染文本为纹理（缓存）
    pub fn render_text(&mut self, size: u16, color: Color, text: &str) -> Result<&Texture<'a>, String> {
        let key = (size, rgb_key(color), text.to_string());
        if !self.textures.contains_key(&key) {
            self.font(size)?;
            let font = self.fonts.get(&size).ok_or(format!("字号 {size} 初始化失败"))?;
            let surface = font.render(text).blended(color).map_err(|e| format!("文字渲染失败：{text:?}（{e}）"))?;
            let mut tex = self
                .creator
